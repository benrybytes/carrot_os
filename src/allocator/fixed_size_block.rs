...
impl FixedSizeBlockAllocator {
    pub const fn new() -> Self {
        // constant can be copied, while direct value cannot
...
fn list_index(layout: &Layout) -> Option<usize> {
    // get block size for allocation of layout, and see which fixed block size works
        ...
        match list_index(&layout) {
            Some(index) => {
                match allocator.list_heads[index].take() {
                    Some(node) => {
                        // remember that the nodes are taken, to indicate we are taking a block,
